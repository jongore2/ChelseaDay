...
if((data.data_version||1)<3){
 ...
 const checkpoints=[...data.checkpoints,...additions].map(item=>item.id==='conditional-offer-review'?{...item,description:seed.checkpoints.find(candidate=>candidate.id===item.id).description}:item);
 data={...data,data_version:3,checkpoints};
 persist(data);
}
